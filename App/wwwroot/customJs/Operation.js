function getOperationFromModel() {
    ...
    model.PreoperativeDiagnosis = $("#Operation_PreoperativeDiagnosis").val();
    model.OperationTitle = $("#Operation_OperationTitle").val();
    model.Anaesthesia = $("#Operation_Anaesthesia").val();
    // Handle the image input
    var imageInput = $("#Operation_PerhapsImage")[0];
    //if (imageInput.files.length > 0) {
    //    var imageFile = imageInput.files[0];
    //    // You might want to handle the file (e.g., upload it to the server) as needed
    //    // For now, let's just set the image name in the model
    //    model.PerhapsImage = imageFile.name;
    //}
